...
### Naming Convention — MODERN (Required for all new agents)

Folder is **name-based with NO numeric prefix**. Files inside `rules/` use **file-level numeric sequence** (`00_`, `01_`, `02_`, …). This matches the pattern used by Agent_Builder, Agent_Tuner, and WSR_Email_Composer_Agent.

| # | File Pattern | Purpose | Priority |
|---|--------------|---------|----------|
| 1 | `{Agent_Name}.mdc` | **MAIN ENTRY** - Identity, RISEN framework, workflow overview | HIGHEST |
| 2 | `00_Phase_Orchestration.mdc` | Phase workflow, checkpoints, state management | HIGH |
| 3 | `01_Guidance.mdc` | Patterns, functions, tools, quick reference | HIGH |
| 4 | `02_Mandatory_Stop_Points.mdc` | Interactive enforcement, stop points | HIGHEST |

Plus: `INDEX.md` for navigation.

**Folder:** `r-genie/{Agent_Name}_Agent/` (PascalCase with underscores, `_Agent` suffix; e.g., `Cloud_Success_Architect_Agent/`, `Security_Scanner_Agent/`). **Do NOT prefix with a numeric sequence** (`11_`, `12_`, etc.).

### Example: Agent Builder (Modern) - 4 Files
```
r-genie/Agent_Builder/rules/
├── Agent_Builder.mdc                 # Main entry (no numeric prefix)
...
### Example: Modern 5-File Agent with Template_Configuration
```
r-genie/{Agent_Name}_Agent/rules/
├── {Agent_Name}.mdc                  # Main entry
├── 00_Template_Configuration.mdc     # Optional template-first (if applicable)
...
*(When Template_Configuration is present, shift the other file sequences by +1 so the numeric ordering reflects runtime dependency: Template Config loads before Phase Orchestration.)*

### Naming Convention — LEGACY (Frozen on agents 00–07 + 03-01 only)

Legacy agents `00_Master_Orchestrator_System/` through `07_Error_Analysis_Agent/` (plus `03-01_Dataweave_Agent/`) use the older `{ID}_` numeric-prefix convention (folder prefix + file prefix). These are **frozen in place for backward compatibility** — do not rename them, and **do not use this convention for new agents**.

| # | File Pattern (legacy) | Purpose |
|---|-----------------------|---------|
| 1 | `{ID}_{Agent}.mdc` | Main Entry |
| 2 | `{ID}-00_Phase_Orchestration.mdc` | Phase workflow |
| 3 | `{ID}-01_Guidance.mdc` | Patterns |
| 4 | `{ID}-02_Mandatory_Stop_Points.mdc` | Stop points |

#### Example: Legacy Technical Design Agent (01) - 5 Files
```
r-genie/01_Technical_Design_Agent/rules/
├── 01_Technical_Design.mdc
├── 01-00_Template_Configuration.mdc
...
**Note**: Legacy agents 01, 02, 05, 06, 07 include `Template_Configuration.mdc` for template-based workflows (5 rule files total).
...
### Modern (Required for new agents)

```
r-genie/{Agent_Name}_Agent/
...
└── Production_Learnings.md             # Lessons learned (no ID prefix)
```

### Legacy (Frozen on agents 00–08)

```
r-genie/{ID}_{Name}_Agent/
├── rules/
│   ├── {ID}_{Name}.mdc
...
**Modern (no `{ID}-` prefix):**
```
See `@00_Phase_Orchestration.mdc`
Reference `lib/docs/{file}.md`
```

**Legacy (agents 00–08 keep `{ID}-` prefix):**
```
...
| Patterns | `@01_Guidance.mdc` |
| Stop points | `@02_Mandatory_Stop_Points.mdc` |
| Detailed docs | `lib/docs/` |
```

**Legacy convention (agents 00–08 only):**
```markdown
## QUICK REFERENCE
...
- [ ] Activation file in `.windsurf/workflows/use-{agent-slug}.md` (e.g., `use-cloud-success-architect.md` — **no numeric prefix** for modern agents; legacy agents 00–08 keep `use-XX-agent-name.md`)
- [ ] Activation file is synced with `.cursor/commands/use-{agent-slug}.md`
...
### Legacy Agents (frozen `{ID}_` naming — do not rename)
...
### Modern Agents (name-based folder, no numeric prefix)
...
**New agents MUST follow the modern naming convention.** Do not introduce numeric prefixes (`11_`, `12_`, …) for new agents — those sequence numbers are reserved-and-frozen on the legacy agent set only.
...
| **S - STEPS** | [N]-phase workflow with STOP points. See `@00_Phase_Orchestration.mdc` (modern) or `@{ID}-00_Phase_Orchestration.mdc` (legacy agents 00–08) |
...
6. Production learnings applicable: [check `Production_Learnings.md` (modern) or `{ID}_Production_Learnings.md` (legacy)]
</thinking>
```
...
| IDE | Activation Files (modern) | Activation Files (legacy 00–08) | Ignore File | Rules Directory |
|-----|---------------------------|---------------------------------|-------------|----------------|
| **Cursor** | `.cursor/commands/use-{agent-slug}.md` | `.cursor/commands/use-XX-agent.md` | `.cursorignore` | `.cursor/rules/` |
| **Windsurf** | `.windsurf/workflows/use-{agent-slug}.md` | `.windsurf/workflows/use-XX-agent.md` | `.windsurfignore` | `.windsurf/rules/` |
...
```markdown
# [Agent Name] Agent *(legacy agents show `(XX)` in heading; modern agents omit the numeric tag)*
...
**Step 1: Setup**
[Tool-first Step 1: `mkdir -p project/input_{agent-slug} project/output_{agent-slug}` + use the `edit` tool on `.windsurfignore`/`.cursorignore` to flip `/r-genie/{Agent_Name}_Agent/` → `# /r-genie/{Agent_Name}_Agent/` (comment = enabled). Do NOT use `sed` — see Gap #15 Tool Misuse Hazard.]

**Step 2: Activate Agent**
- Modern: Load agent rules: `@r-genie/{Agent_Name}_Agent/rules/{Agent_Name}.mdc`
- Legacy (00–08 only): `@r-genie/{ID}_{Agent_Name}_Agent/rules/{ID}_{Agent_Name}.mdc`
...
I want to [create/update] the [Agent Name] Agent *(modern: name-based only, no ID; legacy 00–08: include `(ID: XX)`)*.
...
- [ ] Command file in `.cursor/commands/use-{agent-slug}.md` (modern; or `use-XX-agent-name.md` for legacy agents 00–08)
- [ ] Workflow file in `.windsurf/workflows/use-{agent-slug}.md` (synced with Cursor)
...
- [ ] Mermaid diagrams (Phase Orchestration state machine, INDEX.md Rule Dependency Map + Phase Workflow, ARCHITECTURE.md flow diagram, any domain deliverables) comply with Section 16 Mermaid Diagram Standards

> **Full Templates**: See `.cursor/commands/update-agents/lib/agent-creation-patterns.md`

---

## 16. MERMAID DIAGRAM STANDARDS

> **Canonical reference:** `lib/docs/mermaid-diagram-best-practices.md` — the single source of truth for Mermaid mechanics across the R-GENIE framework.

Every R-GENIE agent that emits Mermaid diagrams (which is all of them — the standard skeleton produces a Phase Orchestration state machine, INDEX.md Rule Dependency Map, INDEX.md Phase Workflow, and ARCHITECTURE.md flow diagram) MUST comply with the ten non-negotiable rules below. Agents whose deliverables include architecture/HLD/flow diagrams (Document Generator archetype, integration-adjacent agents) have additional surfaces and need the full standard even more.

### 16.1 Ten Non-Negotiable Rules (summary)

1. Use `<br/>` for line breaks in labels. **NEVER** `\n`.
2. Every `flowchart`/`graph` starts with `%%{ init: { 'flowchart': { 'curve': 'linear' } } }%%`.
3. Declare nodes in upstream → downstream order.
4. List connections primary-path → branching-path.
5. Blank lines between distinct connection groups.
6. Solid `-->` = forward; dashed `-.->` = return/async. Never mix semantics within a diagram.
7. Short action/protocol/event edge labels; no redundant labels on fan-out arrows.
8. All `style` declarations at the end, in node-declaration order.
9. Quote labels containing spaces, parentheses, slashes, or HTML.
10. No dangling nodes — every node has at least one connection.

> **Full detail** including the init directive, layout control, arrow semantics, edge label discipline, subgraph patterns, Material Design 200/300 colour palette, state-diagram / sequence-diagram specifics, and a pre-emit self-check: `lib/docs/mermaid-diagram-best-practices.md`.

### 16.2 Integration Requirements for Agents

| Agent Surface | Requirement |
|---------------|-------------|
| Phase Orchestration state machine | `stateDiagram-v2`; states in phase order; `[*]` for start/end; transitions labelled with trigger events |
| INDEX.md Rule Dependency Map | `flowchart TD`; init directive; Main Entry first; styles at end |
| INDEX.md Phase Workflow | `flowchart LR`; init directive; phases in execution order |
| ARCHITECTURE.md flow diagram | `flowchart LR`; init directive; workflow steps upstream→downstream |
| Domain deliverable diagrams | All ten rules; domain palette via Material Design 200/300 |

### 16.3 Agent-Level Integration (MANDATORY for New Agents)

Every newly generated agent MUST:
1. Carry a **Mermaid Diagram Standards section** in its Guidance rule file — template in Agent Builder's `01_Guidance.mdc` §12.2.
2. Carry a **MERMAID RENDERING GUARD** item in its Stop Points SELF-CHECK — template in Agent Builder's `01_Guidance.mdc` §12.4.
3. Either **reference** `@r-genie/Agent_Builder/lib/docs/mermaid-diagram-best-practices.md` OR **copy** the doc to its own `lib/docs/` for self-contained deployment.
4. Include the Mermaid Diagram Standards check in its Phase Validation table (Agent Builder §6A).

### 16.4 Enforcement

The Agent Builder enforces these standards via `01_Guidance.mdc` §12, `00_Phase_Orchestration.mdc` Phase 3C/3D/3F/5/6A, `02_Mandatory_Stop_Points.mdc` MERMAID RENDERING GUARD + MERMAID EMBEDDING GUARD, and skeleton-templates.md §2/§7. The Agent Tuner applies the same standards retroactively via the "Mermaid Standards Retrofit" customization pattern in its `customization-patterns.md`.
